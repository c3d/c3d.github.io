...
{
    stime = time * 0.21;
    tc=gl_FragCoord.xy/resolution.xy*2.0-vec2(1.0);
	
    float ti=floor(stime/t_per_target);
    float tf=fract(stime/t_per_target);
	
    vec3 camo=cameraPos(stime);
    ...
    float zoom=mix(cameraZoom(ti-1.0),cameraZoom(ti),smoothstep(0.7,0.9,tf)) * (0.5 + 0.9 * mouse.y);
    ...
    float icing=1.0-mouse.x*icing_factor;
