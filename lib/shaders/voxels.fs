...
float noise( in vec2 x )
{
    ...
    return texture2D( texture, (uv+118.4)/256.0, -100.0 ).x;
}
...
float castRay( in vec3 ro, in vec3 rd, out vec3 oVos, out vec3 oDir )
{
    ...
    for( int i=0; i<128; i++ ) 
    {
        if( map(pos)>0.5 ) { res=1.0; continue; }
...
float castVRay( in vec3 ro, in vec3 rd, in float maxDist )
{
    ...
    for( int i=0; i<50; i++ ) 
    {
        if( map(pos)>0.5 ) {res=0.0; continue; }
...
void main( void )
{
    ...
    if( t>0.0 )
    {
        ...
        col = 2.0*texture2D( texture1,0.01*pos.xz ).zyx; 
        col += 0.8*vec3(0.1,0.3,0.4);
        col *= 0.5 + 0.5*texcube( texture2, 0.5*pos, nor ).x;
