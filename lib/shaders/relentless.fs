...
void main(void)
{
    ...
    // using an iq styled camera this stime :)
    // ray origin
    vec3 ro = 0.7 * vec3(cos(0.2 * stime), 0.0, sin(0.2 * stime));
    ro.y = cos(0.6 * stime) * 0.3 + 0.65;
    // camera look at
    vec3 ta = vec3(0.0, 0.2, 0.0);
	
    // camera shake intensity
    float shake = clamp(3.0 * (1.0 - length(ro.yz)), 0.3, 1.0);
    float st = mod(stime, 10.0) * 22.0;
	
    // build camera matrix
    vec3 ww = normalize(ta - ro + noise31(st) * shake * 0.01);
    vec3 uu = normalize(cross(ww, normalize(vec3(0.0, 1.0, 0.2 * sin(stime)))));
    ...
    for(int i = 0; i < 4; i ++)
    {
        ...
        if(its.x > 0.0)
        {
            ...
            if(i == 3)
            {
                float crd = 0.0;//fract(stime * 0.2) * 50.0 - 25.0;
                float fxi = cos(vo.x * 0.2 + stime * 1.5);//abs(crd - vo.x);
    ...
    inten *= 0.4 + (sin(stime) * 0.5 + 0.5) * 0.6;
	
    // find a color for the computed intensity
    vec3 col = pow(vec3(inten), vec3(9.0, 1.15, 0.3));
